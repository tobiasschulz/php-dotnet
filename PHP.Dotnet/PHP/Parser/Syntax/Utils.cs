...
namespace Devsense.PHP.Syntax
{
    ...
    {
        public static Text.Span2 SafeSpan(int start, int length) => start < 0 || length < 0 ? Text.Span2.Invalid : new Text.Span2(start, length);

        public static Text.Span2 SafeCombineSpan(Text.Span2 previous, Text.Span2 next)
        {
            if (!previous.IsValid)
            {
                return next;
            ...
            {
                return previous.StartOrInvalid <= next.End ? Text.Span2.Combine(previous, next) : Text.Span2.Invalid;
            }
        }

        public static Text.Span2 CombineValid(Text.Span2 s1, Text.Span2 s2, Text.Span2 s3 = default(Text.Span2))
        {
            var result = s1;

            if (s2.Length > 0)
            {
                result = result.IsValid ? Text.Span2.Combine(result, s2) : s2;
            }

            if (s3.Length > 0)
            {
                result = result.IsValid ? Text.Span2.Combine(result, s3) : s3;
            }

            return result;
        }

        public static Text.Span2 SpanIntermission(Text.Span2 previous, Text.Span2 next) =>
            previous.IsValid && next.IsValid && previous.End <= next.Start ? Text.Span2.FromBounds(previous.End, next.Start) : Text.Span2.Invalid;

        public static Text.Span2 SpanIntermission(int previous, Text.Span2 next) =>
            previous >= 0 && next.IsValid && previous <= next.Start ? Text.Span2.FromBounds(previous, next.Start) : Text.Span2.Invalid;

        public static Text.Span2 SpanIntermission(Text.Span2 previous, int next) =>
            previous.IsValid && next >= 0 && previous.End <= next ? Text.Span2.FromBounds(previous.End, next) : Text.Span2.Invalid;

        public static Text.Span2 ItemSpan(this Ast.Item item) => SafeCombineSpan(
            item.HasKey ? item.Index.Span : Text.Span2.Invalid,
            item is Ast.ValueItem ? ((Ast.ValueItem)item).ValueExpr.Span : ((Ast.RefItem)item).RefToGet.Span);

        public static Text.Span2 ItemsSpan(this IList<Ast.Item> items)
        {
            Ast.Item first = null;
            Ast.Item last = null;
