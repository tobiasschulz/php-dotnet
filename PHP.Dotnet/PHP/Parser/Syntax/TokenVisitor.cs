...
namespace Devsense.PHP.Syntax
{
    ...
    public interface ITokenComposer
    {
        ...
        void ConsumeToken(Tokens token, string text, Span2 position);
    }
    ...
    public class DefaultTokenComposer : ITokenComposer
    {
        ...
        /// <summary>
        /// Shortcut for <see cref="ConsumeToken(Tokens, string, Span2)"/>.
        /// </summary>
        protected void ConsumeToken(Tokens token, Span2 position) => ConsumeToken(token, TokenFacts.GetTokenText(token), position);

        /// <inheritdoc />
        public virtual void ConsumeToken(Tokens token, string text, Span2 position)
        {
    ...
    public class TokenVisitor : TreeContextVisitor
    {
                ...
                else if (literal.ContainingElement is ShellEx)
                {
                    ...
                    if (value.Length != 0)
                    {
                        ConsumeToken(
                            ...
                            value,
                            literal.Span.IsValid ? SpanUtils.SafeSpan(literal.Span.Start + 1, literal.Span.Length - 2) : Span2.Invalid);
                    }
        ...
        protected virtual void ConsumeModifiers(LangElement element, PhpMemberAttributes modifiers, ISourceToken[] tokens, Span2 span)
        {
            tokens.Foreach(ConsumeToken);
        }

        private ISourceToken ProcessToken(Tokens target, Span2 span)
        {
            var token = _provider.GetTokenAt(span, target, new SourceToken(target, Span2.Invalid));
            ConsumeToken(token);
            return token;
        }

        private ISourceToken ProcessToken(Tokens target, string text, Span2 span)
        {
            var token = _provider.GetTokenAt(span, target, new SourceToken(target, Span2.Invalid));
        ...
        public void ConsumeToken(Tokens token, string text, Span2 position)
        {
            ...
            Dictionary<Tokens, ISourceToken> defaults)
        {
            ...
            if ((modifiers & PhpMemberAttributes.VisibilityMask) == PhpMemberAttributes.Public)
                defaults.Add(token, new SourceToken(token, Span2.Invalid));
        }
        ...
        protected virtual ISourceToken[] ConsumeModifiers(LangElement element, PhpMemberAttributes modifiers, Span2 span)
        {
        ...
        protected virtual void VisitItemList(IList<Item> list, Tokens separatorToken, ISourceToken previous, Span2 terminal)
        {
        ...
        private void ConsumeLogicalOperator(Span2 span, Tokens symbolic, Tokens verbose)
        {
            ISourceToken token = new SourceToken(symbolic, Span2.Invalid);
        ...
        public override void VisitBlockStmt(BlockStmt x)
        {
            if (x is ColonBlockStmt block)
            {
                ...
                ConsumeToken(block.OpeningToken, openingText, new Span2(x.Span.StartOrInvalid, openingText.Length));
        ...
        public override void VisitExitEx(ExitEx x)
        {
            ...
            var paren = _provider.GetTokenAt(x.ResulExpr != null ? SpanUtils.SpanIntermission(token.Span, x.ResulExpr.Span) : x.Span, Tokens.T_LPAREN,
                ExitHasParentheses ? new SourceToken(Tokens.T_LPAREN, Span2.Invalid) : null);
            if (paren != null)
            {
            ...
            paren = _provider.GetTokenAt(x.ResulExpr != null ? SpanUtils.SpanIntermission(x.ResulExpr.Span, x.Span.End) : x.Span, Tokens.T_RPAREN,
                ExitHasParentheses ? new SourceToken(Tokens.T_RPAREN, Span2.Invalid) : null);
            if (paren != null)
            {
                ConsumeToken(paren.Token, paren.Span);
            }
        ...
        protected virtual void VisitRoutineDecl(LangElement element, Span2 headerSpan, Signature signature, Statement body, NameRef nameOpt = default(NameRef), TypeRef returnTypeOpt = null, PhpMemberAttributes modifiers = PhpMemberAttributes.None, IList<FormalParam> useParams = null)
        {
            if (!headerSpan.IsValid)
            {
                ...
                headerSpan =
                    ...
                    element.Span.IsValid ? new Span2(element.Span.Start, 0) : Span2.Invalid,
                    signature.Span,
                    returnTypeOpt != null ? returnTypeOpt.Span : (useParams != null && useParams.Count != 0) ? useParams.Last().Span : Span2.Invalid);
            }
        ...
        public virtual void VisitVariableName(VariableName name, Span2 span, bool dollar, bool isEncapsed)
        {
        ...
        private static List<ISourceToken> BuildQnameDefaults(QualifiedName qname, Span2 span, bool isNamespace)
        {
        ...
        public virtual void VisitQualifiedName(QualifiedName qname, Span2 span, bool isNamespace = false)
        {
        ...
        public virtual void VisitIsMemberOf(Expression isMemberOf, Span2 next)
        {
            if (isMemberOf != null)
            {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                if (i != 0) ProcessToken(separatorToken, separatorTokenText,
                    SpanUtils.SpanIntermission(
                        list[i - 1] != null ? list[i - 1].Span : Span2.Invalid,
                        list[i] != null ? list[i].Span : Span2.Invalid));
                VisitElement(list[i]);
            }
        }
        ...
        public override void VisitSwitchStmt(SwitchStmt x)
        {
            ...
            var separator = _provider.GetTokenAt(braceSpan, Tokens.T_COLON, null) ??
                ...
                SwitchShortNotation ? new SourceToken(Tokens.T_LBRACE, Span2.Invalid) : new SourceToken(Tokens.T_COLON, Span2.Invalid));
        ...
        private void ConsumeNameToken(string value, Span2 span)
        {
        ...
        public override void VisitWhileStmt(WhileStmt x)
        {
            ...
            else
            {
                ...
                VisitBody(x.Body);
                Span2 whileSpan = SpanUtils.SpanIntermission(x.Body.Span, x.CondExpr.Span);
