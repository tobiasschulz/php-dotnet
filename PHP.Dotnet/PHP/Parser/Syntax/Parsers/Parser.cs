...
namespace Devsense.PHP.Syntax
{
    ...
    {
        public readonly Span2 Span;
        public readonly LangElement Condition;
        public readonly LangElement Body;

        public IfStatement(Span2 span, LangElement condition, LangElement body)
        {
            Span = span;
            Condition = condition;
            Body = body;
    ...
    {
        IParserTokenProvider<SemanticValueType, Span2> _lexer;
        INodesFactory<LangElement, Span2> _astFactory;
        IErrorSink<Span2> _errors;
        ...
        /// </summary>
        public IErrorSink<Span2> ErrorSink => _errors;
        ...
        protected override Span2 InvalidPosition => Span2.Invalid;

        protected override Span2 CombinePositions(Span2 first, Span2 last)
        {
            if (last.IsValid)
            {
                if (first.IsValid)
                    return Span2.Combine(first, last);
                else
                    return last;
            }
            else
        ...
        public LangElement Parse(
                ITokenProvider<SemanticValueType, Span2> lexer,
                INodesFactory<LangElement, Span2> astFactory,
                LanguageFeatures language,
                IErrorSink<Span2> errors = null,
                ...
                int positionShift = 0)
        {
            ...
            _errors = errors ?? new EmptyErrorSink<Span2>();
        ...
        Span2 SignatureSpan(IList<FormalParam> parameters)
        {
                    ...
                    if (ns.IsSimpleSyntax)
                    {
                        ...
                        if (i + 1 < statements.Count)
                        {
                            ...
                            if (count != 0)
                            {
                                ...
                                Span2 newSpan = newcontaining.Length > 0 ? CombineSpans(newcontaining.First().Span, newcontaining.Last().Span) : new Span2(ns.Span.End, 0);
        ...
        private BlockStmt EmptyNamespaceBody(Span2 position)
        {
            return (BlockStmt)_astFactory.SimpleBlock(new Span2(position.End, 0), EmptyArray<Statement>.Instance);
        }

        private SimpleUse AddAlias(Tuple<QualifiedNameRef, NameRef> alias)
        {
            return AddAlias(new Tuple<Span2, QualifiedNameRef, NameRef>(CombineSpans(alias.Item1.Span, alias.Item2.Span), alias.Item1, alias.Item2), _contextType);
        }

        private SimpleUse AddAlias(Tuple<Span2, QualifiedNameRef, NameRef> alias, AliasKind contextType)
        {
            var aliasName = alias.Item3.HasValue
                ? alias.Item3
                : new NameRef(Span2.Invalid, alias.Item2.QualifiedName.Name);
        ...
        private GroupUse AddAliases(Span2 span, List<string> prefix, Span2 prefixSpan, List<Tuple<QualifiedNameRef, NameRef>> aliases, bool isFullyQualified)
        {
        ...
        private GroupUse AddAliases(Span2 span, List<string> prefix, Span2 prefixSpan, List<Tuple<Span2, QualifiedNameRef, NameRef, AliasKind>> aliases, bool isFullyQualified)
        {
            List<SimpleUse> uses = new List<SimpleUse>();
            var prefixNamespaces = prefix.Select(p => new Name(p));
            foreach (var alias in aliases)
            {
                ...
                uses.Add(AddAlias(new Tuple<Span2, QualifiedNameRef, NameRef>(
        ...
        private LangElement StatementsToBlock(Span2 span, List<LangElement> statements, Tokens endToken)
        {
            _lexer.DocBlockList.Merge(span, statements, _astFactory);
            return _astFactory.ColonBlock(span, statements, endToken);
        }

        private LangElement StatementsToBlock(Span2 span, Span2 endSpan, object statements, Tokens endToken)
        {
            Debug.Assert(statements is List<LangElement>);
            var statemenList = (List<LangElement>)statements;
            return StatementsToBlock(Span2.FromBounds(span.Start, endSpan.Start), statemenList, endToken);
        }

        void RebuildLast(List<IfStatement> condList, Span2 end, Tokens token)
        {
            var block = condList.Last();
            var colon = ((ColonBlockStmt)block.Body);
            colon.ExtendSpan(Span2.FromBounds(block.Body.Span.Start, end.Start));
            colon.SetClosingToken(token);
            condList.Remove(block);
            condList.Add(new IfStatement(Span2.FromBounds(block.Span.Start, end.Start), block.Condition, block.Body));
        }

        private LangElement CreateProperty(Span2 span, LangElement objectExpr, object name)
        {
        ...
        private LangElement CreateStaticProperty(Span2 span, TypeRef objectName, Span2 objectNamePos, object name)
        {
        ...
        private LangElement CreateStaticProperty(Span2 span, LangElement objectExpr, Span2 objectNamePos, object name) =>
            CreateStaticProperty(span, _astFactory.TypeReference(objectNamePos, objectExpr), objectNamePos, name);

        private Span2 CombineSpans(params Span2[] spans)
        {
            var validSpans = spans.Where(s => s.IsValid);
            return Span2.FromBounds(validSpans.Min(s => s.Start), validSpans.Max(s => s.End));
        }
        ...
        private TypeRef CreateNamedTypeRef(Span2 span, QualifiedName tname)
        {
        ...
        LangElement CreateIncDec(Span2 span, LangElement variable, bool inc, bool post)
        {
        ...
        private Span2 CombineSpans(Span2 a, Span2 b) => a.IsValid ? (b.IsValid ? Span2.Combine(a, b) : a) : b;
        ...
        BlockStmt CreateBlock(Span2 span, List<LangElement> statements)
        {
        ...
        /// <param name="statements">List of statements in the block.</param>
        /// <returns>Complete block statement.</returns>
        BlockStmt CreateCaseBlock(Span2 separatorSpan, List<LangElement> statements)
        {
            if (statements.Count == 0)
                return (BlockStmt)_astFactory.Block(separatorSpan, statements);
            Span2 bodySpan = Span2.Combine(separatorSpan, statements.Last().Span);
