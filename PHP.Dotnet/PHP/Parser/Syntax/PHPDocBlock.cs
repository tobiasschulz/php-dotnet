...
namespace Devsense.PHP.Syntax
{
    ...
    public sealed class PHPDocBlock : LangElement
    {
        ...
        public abstract class Element
        {
            ...
            /// </summary>
            public Span2 Span { get; internal set; }
        ...
        public abstract class TypeVarDescTag : Element
        {
            ...
            public Span2 TypeNamesSpan
            {
                get
                {
                    ...
                    return Span2.FromBounds(offset + positions[0], offset + positions[positions.Length - 1] + names[names.Length - 1].Length);
                }
            }
            ...
            public Span2[]/*!*/TypeNameSpans
            {
                get
                {
                    ...
                    Span2[] spans = new Span2[positions.Length];
                    var offset = this.Span.Start;
                    for (int i = 0; i < spans.Length; i++)
                        spans[i] = new Span2(offset + positions[i], names[i].Length);
            ...
            public Span2 VariableNameSpan
            {
                get
                {
                    ...
                    return new Span2(this.Span.Start + this._variableNameOffset, this.VariableName.Length);
                }
            }
        ...
        public sealed class MethodTag : Element
        {
            ...
            public Span2 MethodNameSpan
            {
                get
                {
                    ...
                    return new Span2(pos + this.Span.Start, this.MethodName.Length);
                }
            }
            ...
            private static FormalParam/*!*/ParseParam(string/*!*/paramDecl)
            {
                ...
                Span2 paramnamespan = Span2.Invalid;
                bool byref = false;
                ...
                if (eqIndex >= 0)
                {
                    initExpr = TryParseInitValue(Span2.Invalid, paramDecl.Substring(eqIndex + 1).Trim());
                ...
                if (word != null)
                {
                    ...
                    if (word.Length > 0 && word[0] != '$')
                    {
                        ...
                        {
                            typehint = TypeRef.FromString(new Span2(i - word.Length, word.Length), word);    // TODO: naming
                        }
                ...
                return new FormalParam(
                    Span2.Invalid, paramname, paramnamespan, typehint,
            ...
            private static Expression TryParseInitValue(Span2 span, string value)
            {
        ...
        public PHPDocBlock(string doccomment, Span2 span)
            : base(span)
        {
            ...
            for (int lineIndex = 0; lineIndex < lineBreaks.LinesCount; lineIndex++)
            {
                ...
                if (Element.TryParseLine(ref line, out tmp, lineIndex, out startCharIndex, out endCharIndex))    // validate the line, process tags
                {
                    ...
                    if (tmp == null)    // no new element created
                    {
                        ...
                        if (current.Span.IsValid == false)      // ShortDescriptionElement has not initialized Span
                        {
                            if (!current.IsEmpty)   // initialize Start iff element has some text
                                current.Span = new Span2(offset + lineSpan.Start + startCharIndex, endCharIndex - startCharIndex);
                        }
                        else                                    // other elements has to update their end position
                        {
                            if (tmp != null)
                                current.Span = new Span2(current.Span.Start, offset + lineSpan.Start + endCharIndex - current.Span.Start);   // update its end position                        
                        }
                    }
                    ...
                    if (tmp != null)    // new element created, it is already initialized with the current line
                    {
                        ...
                        tmp.Span = new Span2(offset + lineSpan.Start + startCharIndex, endCharIndex - startCharIndex);
